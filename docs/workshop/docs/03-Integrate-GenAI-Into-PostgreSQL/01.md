...
!!! task "Select the tab for the method you would like to use to allowlist the extensions and follow the instructions provided."

=== "Azure CLI"

    1. In VS Code, open a new integrated terminal window and execute the following Azure CLI command at the prompt.
...
With the required extensions added to the _allowlist_, you are ready to install them in your database. To enable them, you will run a [CREATE EXTENSION](https://www.postgresql.org/docs/current/sql-createextension.html) command for each in PostgreSQL.
...
!!! task "Select each of the tabs below and execute the `CREATE EXTENSION` command to install the extensions."
