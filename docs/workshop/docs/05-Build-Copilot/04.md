...
In this step, you will review backend API code for the `/chat` endpoint in the `completions` router, along with the various functions used for implementing the RAG pattern. You will then add the `completions` router to the FastAPI application to make the `/chat` endpoint available.

## Review Chat Endpoint Implementation

The _Woodgove Bank API_ exposes endpoints in various _routers_... The `chat` endpoint resides in the `completions` router, defined in the `src/api/app/routers/completions.py` file. Open it now in VS Code and explore the code in sections. You can also expand the section belwot see the code inline and review explanations for each line of code.
...
2. **Intialize the router** (lines 10-15): The `completions` router, assigning the route _prefix_, dependencies, and other metadata.

3. **Define the chat endpoint** (line2 17-23): The `/chat` endpoint is the entry point into the _Woodgove Bank_ copilot implementation. It expects a `CompletionRequest`, which contains the user query, the chat history and the maximum number of history messages to include in the prompt, and returns a text response.
...
4. **Chat endpoint implementation** (lines 24-78). The "/completions/chat" route maps to the endpoint where we can invoke the Contoso Chat implementation.
      ...
      - **Build messages collection** (lines 30-35): The messages collection provides the system and user prompts to the LLM, along with chat history messages. Each message consists of a `role` and message `content`. The role will be one of `system`, `assistant`, or `user`. After the `system` message, all subsequent messages must be `user` / `assistant` pairs, with a user query followed by an assistant response.
      ...
      - **Build the model prompt** (lines 38-45): The LangChain `ChatPromptTemplate` class allows you to build a _model prompt_ from a collection of messages.
        ...
        - The _chat history_ is inserted as context of previous questions and responses.
        - The _user input_ provides the model with the current question it is attempting to answer.
        - An _agent scratchpad_ placeholder is included to allow responses from tools assigned to the agent to _augment_ the model with grounding data from.
      ...
      - **Implement function calling** (lines 48-72):
        - Line 48 instantiates the `ChatFunctions` class, which contains the methods used for interacting with the PostgreSQL database. You can review the functions in the `src/api/app/functions/chat_functions.py` file.
        ...
        - Tools are created using the `StructuredTool.from_function` method provided by LangChain.
            ...
            ??? info "About the LangChain `StructuredTool` class"
                ...
                Using the `StructuredTool.from_function` method automates the creation of the JSON function definitions, simplifying the process of function calling when using LangChain.
      ...
      - **Create a LangChain agent** (lines 75-76): The LangChain agent is responsible for interacting with the LLM to generate a response.
         ...
         - LangChain's `AgentExecutor` class is used to manage the execution flow of the agent. It handles the processing of inputs, the invocation of tools or models, and the handling of outputs.
      ...
      - **Invoke the agent** (line 77): Using the agent executor's async `invoke` method, the incoming user message, chat history is sent to the LLM.

        - The `input` and `chat_history` tokens were defined in the prompt object created using the `ChatPromptTemplate`. With the `invoke` method, these will be injected into the model prompt, allowing the LLM to use that information when creating a response.
...
3. Insert the following code at the end of that block to add the `completions/chat` endpoint to the exposed API.
