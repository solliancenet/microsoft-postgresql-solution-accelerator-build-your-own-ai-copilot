...
The initial prompt used by your copilot looks like:
...
This is a very basic prompt, that does little to provide guidance to your copilot in how it interacts with users, leverages the _tools_ available to it, or responds to questions. Now, you are going to iterate on that prompt to improve how it interacts with users and the types of responses it is able to provide.

Prompt engineering typically involves an iterative process of making small changes the prompt, then testing their impact. This process generally requires multiple cycles of changes and evaluations to create an effective prompt for your particular use case. The iterative processs generally follow steps similar to the following:
...
You will using the active VS Code debug sessions for both the API and the Portal you started in the last task to test a few changes to the copilot's prompt and evaluate the responses you are receiving.

1. Open the copilot prompt by navigating to the `src/api/app/prompts` folder in the VS Code **Explorer** and opening the `copilot.txt` file you find there.
...
4. In VS Code, use the floating debugger toolbar to restart the **API Dugger** session. Ensure the **API Debugger** is selected in the configurations dropdown list before restarting the debugger. Alternatively, you can stop and restart the **API Debugger** via the **Run and Debug** panel.
...
9. Return to the _Woodgrove Bank Contract Management Portal_, and some questions about vendor performance, accuracy, and issues with billing. For example, you can ask, "What vendor has had the most invoicing issues?"
...
!!! success "You have successfully implemented an intelligent copilot!"
