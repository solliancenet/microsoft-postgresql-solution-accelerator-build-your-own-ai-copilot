...
1. Open a new integrated terminal in VS Code. Then, execute the following command:
    ...
    In other words, use `and up` when setting everything up from the beginning and `azd deploy` when updating an existing deployment.
...
3. In the _Woodgrove Bank Contract Management Portal_, select the **Dashboard** page and use the copilot to ask a few questions.
