...
5. **Connect to PostgreSQL** (lines 25-28): The `connect` method of the `factory` opens a connection to your Azure Database for PostgreSQL flexible server.
      ...
      2. The `get_connection_string()` function is defined on lines 49-60.
...
6. **Download CSV files from blob storage** (line 34): The CSV files you created in the previous task are downloaded from blob storage and written into a local folder, where the graph loader can easily access them.
      ...
      1. The `download_csvs()` function is defined on lines 62-75. This function creates a `BlobServiceClient` instance, which is used to retrieve the blobs in your storage account's `graph` container.
      2. For each blob with the extension of `.csv`, the `download_csv` function defined on lines 77-93 is used to retrieve the blob's contents and write them into a local file.

7. **Create and load the graph database** (lines 38-52): The `load` method of the `factory` does the following:
...
8. **Define the main guard** (lines 102-109): The main guard defines how the `graph_loader` is executed when called directly. This code block lets you run the script from a command line or VS Code debugging session.
