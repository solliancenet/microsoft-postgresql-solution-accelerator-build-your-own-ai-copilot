...
async def validate_invoice_by_id(id: int, llm = Depends(get_chat_client)):
    ...
    userMessage = "validate Invoice with ID {id}"
...
async def validate_invoice(id: int):
    ...
    async with pool.acquire() as conn:
        ...
        # Get the vendor name
        vendor = await conn.fetchrow('SELECT * FROM vendors WHERE id = $1;', invoice.vendor_id)
        invoice.vendor = parse_obj_as(Vendor, dict[row])
        ...
        for milestone in sow.milestones:
            ...
            milestone.deliverables = [row['description'] for row in deliverable_rows]
        
        # Get the line items
        line_item_rows = await conn.fetch('SELECT * FROM line_items WHERE invoice_id = $1;', id)
