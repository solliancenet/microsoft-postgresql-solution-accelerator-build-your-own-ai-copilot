from app.lifespan_manager import get_pool
...
async def get(pool = Depends(get_pool)):
...
async def get_by_id(id: int, pool = Depends(get_pool)):
...
async def get_by_type(type: str, pool = Depends(get_pool)):
